...
function updateData() {
  ...
  func.getApi(function (data) {
    ...
    for (let i = 0; i < data.data.length; i++) {
      ...
      // function truncateApi(){
      // LiveData.deleteMany({},(err)=>{
      //   if(err){
      //     console.log(err);
      //   }
      // });
...
//---------POST MYENTRY----delete transaction--------
...
app.get("/welcome", isAuth.ensureAuthenticated, function (req, res) {
  User.findById({ _id: req.user._id }, function (err, foundUser) {
    if (err) {
      console.log(err);
    } else {
      if (foundUser) {
        res.render("welcome", { name: foundUser.fName, user: foundUser })
      }
    }
  });
});
